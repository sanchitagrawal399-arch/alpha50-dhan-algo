...
import dhanhq  # Importing the complete module securely
...
# ==============================================================================
# 🛡️ DHAN CONNECTION & TRADER CONTROL INSTANCE (GLOBAL SCOPE FIX)
# ==============================================================================
try:
    # 1. Initialize DhanContext globally
    ctx = dhanhq.DhanContext(client_id=CLIENT_ID, access_token=ACCESS_TOKEN)
    
    # 2. Directly initialize TraderControl wrapper since dhanhq main class was stripped
    dhan = dhanhq.TraderControl(dhan_context=ctx)
    print("🎯 Connection handshake established successfully via TraderControl.")
except Exception as e:
    print(f"❌ Connection Mapping Failure: {e}")
    exit()
...
try:
    # Fetching profile through global trader instance mapping
    ...
    if profile and profile.get('status') == 'success':
        ...
        print(f"⚠️ Profile check bypass, response received: {profile}")
except Exception as e:
    print(f"❌ Dhan API Execution Error: {e}")
    # Handshake is successful, we can inspect profile layout in next cycle if needed
    pass
